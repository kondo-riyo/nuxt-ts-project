  ...
  <nav class="sm:space-x-10 sm:space-y-5 sm:w-10 mt-0 bg-white">
    <div class="flex flex-row sm:flex-col items-end">
      ...
      <div
        ...
        class="sm:block sm:mb-13 sm:mt-10 text-center font-bold self-end sm:self-center text-base_red"
      >
        {{userInfoFromStore.name }}
      </div>
      <!-- <span class="flex flex-row justify-end bg-red-100 w-4/6"> -->
      <div v-if="userInfoFromStore" class="block w-1/6 sm:w-full sm:mb-3 sm:mt-20 text-center justify-self-end">
      ...
      <div class="block w-1/6 sm:w-full sm:mb-3 sm:my-10 font-bold inline-block bottom-0 right-0 text-center" v-if="userInfoFromStore">
        <router-link to="/OrderLog">
         <img src="~/assets/img/rireki.png" class="w-10 sm:w-1/3 inline-block" />
        </router-link>
      </div>
      <div v-if="!userInfoFromStore" class="block sm:mb-3 font-bold text-center">
        <router-link to="/signup">会員登録</router-link>
      </div>
      <div title="ログイン" class="block sm:mb-3 sm:my-10 font-bold text-center" v-if="!userInfoFromStore">
        <router-link to="/signin">
         <img src="~/assets/img/login_icon.png" class="w-10 sm:w-1/3 inline-block" />
        </router-link>
      </div>
      <div title="ログアウト" class="block sm:mb-3 sm:my-10 w-1/6 sm:w-full text-center justify-self-end" v-if="userInfoFromStore">
        <router-link to="/">
